...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if PgNode_a(action).prenodes.issubset(s_nodes):
                a_nodes.add(a_node)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for a1_effect_pos in node_a1.action.effect_add:
            for a2_effect_neg in node_a2.action.effect_rem:
                if a1_effect_pos == a2_effect_neg:
                    return True

        for a1_effect_neg in node_a1.action.effect_rem:
            for a2_effect_pos in node_a2.action.effect_add:
                if a1_effect_neg == a2_effect_pos:
                    return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        a1 = node_a1.action
        a2 = node_a2.action
        eff_pos_1 = set(a1.effect_add)
        eff_neg_1 = set(a1.effect_rem)
        eff_pos_2 = set(a2.effect_add)
        eff_neg_2 = set(a2.effect_rem)
        pre_pos_1 = set(a1.precond_pos)
        pre_neg_1 = set(a1.precond_neg)
        pre_pos_2 = set(a2.precond_pos)
        pre_neg_2 = set(a2.precond_neg)

        for pos in eff_pos_1:
            if pos in pre_neg_2:
                return True
        for pos in eff_pos_2:
            if pos in pre_neg_1:
                return True
        for neg in eff_neg_1:
            if neg in pre_pos_2:
                return True
        for neg in eff_neg_2:
            if neg in pre_pos_1:
                return True
